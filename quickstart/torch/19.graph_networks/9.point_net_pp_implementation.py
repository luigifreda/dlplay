...
if __name__ == "__main__":
    ...
    train_dataset = GeometricShapes(
        root=f"{DATA_DIR}/datasets/GeometricShapes", train=True
    )
    train_dataset.transform = T.Compose(
        [T.SamplePoints(num=num_point_samples), T.KNNGraph(k=6)]
    )

    test_dataset = GeometricShapes(
        root=f"{DATA_DIR}/datasets/GeometricShapes", train=False
    )
    ...
    model = PointNet(num_classes=train_dataset.num_classes)
    model.to(device)
    print(model)
    ...
    # Using this setup, you should get around 75%-80% test set accuracy,
    # even when training only on a single example per class.
