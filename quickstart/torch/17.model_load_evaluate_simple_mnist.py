...
if __name__ == "__main__":
    ...
    model.load_state_dict(
        torch.load(
            f"{RESULTS_DIR}/saved_models/trained_mnist_model.pth", weights_only=True
        )
    )
    ...
    model.eval()  # set the model to evaluation mode and disable gradient computation

    # let's test the model on a random set of images
    num_test_samples = 10
    with torch.no_grad():
