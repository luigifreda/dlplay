...
# This is a modified version of the ShapeNet dataset from the PyTorch Geometric library.
# Since the original link is not working, we have to use the Kaggle dataset instead.
class ShapeNet(InMemoryDataset):
    ...
    **STATS:**
    ...
    # Kaggle dataset name for fallback download
    ...
    def download(self) -> None:
        """Download the ShapeNet dataset from Stanford or Kaggle."""
        # First try the original Stanford URL
        try:
        ...
        # Fallback to Kaggle download using kagglehub
        try:
            kaggle_path = kagglehub.dataset_download(self.kaggle_dataset_name)
            print(f"Downloaded from Kaggle to: {kaggle_path}")

            # The Kaggle dataset structure is different, we need to copy the relevant files
            # to our raw directory structure
            self._setup_raw_dir_from_kaggle(kaggle_path)

        except Exception as e:
            print(f"Error downloading from Kaggle: {e}")
            raise RuntimeError(
                "Failed to download ShapeNet dataset from both Stanford and Kaggle. "
                "Please check your internet connection and Kaggle authentication."
            )

    def _setup_raw_dir_from_kaggle(self, kaggle_path: str) -> None:
        """Set up the raw directory structure from Kaggle download."""
        import shutil

        # Create raw directory if it doesn't exist
        os.makedirs(self.raw_dir, exist_ok=True)

        # Find the ShapeNet data directory in the Kaggle download
        shapenet_data_dir = None
        for root, dirs, files in os.walk(kaggle_path):
            if "shapenetcore_partanno_segmentation_benchmark_v0_normal" in dirs:
                shapenet_data_dir = os.path.join(
                    root, "shapenetcore_partanno_segmentation_benchmark_v0_normal"
                )
                break

        if shapenet_data_dir is None:
            raise RuntimeError(
                "Could not find ShapeNet data directory in Kaggle download"
            )

        print(f"Found ShapeNet data at: {shapenet_data_dir}")

        # Copy the entire ShapeNet directory structure to our raw directory
        if os.path.exists(self.raw_dir):
            shutil.rmtree(self.raw_dir)
        shutil.copytree(shapenet_data_dir, self.raw_dir)

        print(f"Copied ShapeNet data to: {self.raw_dir}")
