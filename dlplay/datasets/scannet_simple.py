...
# Define IGNORE_LABEL constant
IGNORE_LABEL = -1

log = logging.getLogger(__name__)

# Uncomment this to see the logging messages
if True:
    logging.basicConfig(
        level=logging.INFO,
...
class ScanNet(InMemoryDataset):
    ...
    def __init__(
        ...
        self.include_normals = include_normals  # Add this line
        super().__init__(
    ...
    def _compute_normals(points):
        ...
        try:
            ...
            # Fallback: return zeros if Open3D is not available
            return np.zeros((points.shape[0], 3), dtype=np.float32)

    @staticmethod
    def read_one_test_scan(scannet_dir, scan_name, normalize_rgb, include_normals=True):
        ...
        if include_normals:
            normals = ScanNet._compute_normals(mesh_vertices[:, :3])
            data["x"] = torch.from_numpy(normals).float()
        else:
            data["x"] = None
    ...
    def read_one_scan(
        ...
        obj_class_ids,
        normalize_rgb,
        include_normals=True,  # Add this parameter
    ):
        mesh_file = osp.join(scannet_dir, scan_name, scan_name + "_vh_clean_2.ply")
    ...
    def process_func(
        ...
        if split == "test":
            data = ScanNet.read_one_test_scan(
                scannet_dir, scan_name, normalize_rgb, include_normals
            )
        else:
            data = ScanNet.read_one_scan(
                ...
                obj_class_ids,
                normalize_rgb,
                include_normals,  # Add this parameter
            )
        log.info(
        ...
        # Replace cT.SaveOriginalPosId() with a simple pass
        # The SaveOriginalPosId transform was likely just saving the original position
        # for later use, but we can skip this for now
        return SaveOriginalPosId()(data)
    ...
    def process(self):
        ...
        for i, (scan_names, split) in enumerate(zip(self.scan_names, self.SPLITS)):
            if not os.path.exists(self.processed_paths[i]) or self.force_reload:
                mapping_idx_to_scan_names = getattr(
