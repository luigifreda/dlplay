...
# ******************************************************************************
from dlplay.models.linear.logistic_regression import MulticlassLogisticRegression
...
def sklearn_baseline(X_train, X_test, y_train, y_test, max_iter=1000):
    ...
    print(f"Sklearn logistic regression - test accuracy: {acc:.4f}")


def main():
    dataset_name = ToyDatasetType.WINE
    ...
    print(f"target classes: {classes}, counts: {counts}")
    ...
    optimizer_vanilla = GradientDescentVanillaOptimizer(
        name="multiclass_logistic_regression_vanilla_gd",
        objective_function=model,
        max_iterations=max_iterations,
        learning_rate=learning_rate,
    )
    optimizer_momentum = GradientDescentMomentumOptimizer(
        name="multiclass_logistic_regression_momentum_gd",
        objective_function=model,
        max_iterations=max_iterations,
        learning_rate=learning_rate,
    )
